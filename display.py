import os
from rich import print as p

class Display:

    # System
    @staticmethod
    def clear_console():
        os.system("cls")

    @staticmethod
    def char_choice():
        pass

    #region ASCII Art
    @staticmethod
    def title():
        p(r"""[red]
 _______  _______  _______           _______  _______  _______  _______ 
|  ____ ||  ___  ||  ____ \|\     /||  ____ \|  ___  ||  ___  ||  ___  |
| |    ||| |   | || |    \/| |   | || |    \/| |   | || |   | || |   | |
| |____||| |   | || |      | |   | || |__    | |___| || |   | || |   | |
|     __|| |   | || | ____ | |   | ||  __|   |  _____|| |   | || |   | |
| |\ \   | |   | || | \_  || |   | || |      | |      | |   | || |   | |
| | \ \__| |___| || |___| || |___| || |____/\| /      | |___| || |___| |
|/   \__/|_______||_______||_______||_______/|/       |_______||_______|
[/red]
""")

    @staticmethod
    def infinite():
        p(r"""[bright_green]
_________ _        _______ _________ _       __________________ _______ 
\__   __/| \    /||  ____ \\__   __/| \    /|\__   __/\__   __/|  ____ \
   | |   |  \  | || |    \/   | |   |  \  | |   | |      | |   | |    \/
   | |   |   \ | || |__       | |   |   \ | |   | |      | |   | |__    
   | |   | |\ \| ||  __|      | |   | |\ \| |   | |      | |   |  __|   
   | |   | | \   || |         | |   | | \   |   | |      | |   | |      
___| |___| |  \  || |      ___| |___| |  \  |___| |___   | |   | |____/\
\_______/|/    \_||/       \_______/|/    \_|\_______/   |_|   |_______/   
[/bright_green]
""")

    @staticmethod
    def dungeon():
        p(r"""[yellow1]
 ______            _        _______  _______  _______  _       
|  __  \ |\     /|| \    /||  ____ \|  ____ \|  ___  || \    /|
| |  \  \| |   | ||  \  | || |    \/| |    \/| |   | ||  \  | |
| |   | || |   | ||   \ | || |      | |__    | |   | ||   \ | |
| |   | || |   | || |\ \| || | ____ |  __|   | |   | || |\ \| |
| |   | || |   | || | \   || | \_  || |      | |   | || | \   |
| |__/  /| |___| || |  \  || |___| || |____/\| |___| || |  \  |
|______/ |_______||/    \_||_______||_______/|_______||/    \_|
[/yellow1]
""")

    @staticmethod
    def castle():
        print(r"""
                                  |>>>
                                  |
                    |>>>      _  _|_  _         |>>>
                    |        |;| |;| |;|        |
                _  _|_  _    \\.    .  /    _  _|_  _
               |;|_|;|_|;|    \\:. ,  /    |;|_|;|_|;|
               \\..      /    ||;   . |    \\.    .  /
                \\.  ,  /     ||:  .  |     \\:  .  /
                 ||:   |_   _ ||_ . _ | _   _||:   |
                 ||:  .|||_|;|_|;|_|;|_|;|_|;||:.  |
                 ||:   ||.    .     .      . ||:  .|
                 ||: . || .     . .   .  ,   ||:   |       \,/
                 ||:   ||:  ,  _______   .   ||: , |            /`\
                 ||:   || .   /+++++++\    . ||:   |
                 ||:   ||.    |+++++++| .    ||: . |
              __ ||: . ||: ,  |+++++++|.  . _||_   |
     ____--`~    '--~~__|.    |+++++__|----~    ~`---,              ___
-~--~                   ~---__|,--~'                  ~~----_____-~'   `~----~~


               You arrived in front of a huge abandonned castle
""")

    @staticmethod
    def room():
        p(r"""
*******#%#********#***********************++*#%++*++**
********%#*******************#*#***********++#%+++++**
+++++++*%*++++**********##%%%##***********+++#%++==+++
*++===+*%*++********#%%@@@@@@@@@@%#*******+++#%+===+**
+++-:-=*%*++*****#@@@%=*%=+#=+%+=@@@%#***++++#%=--:=++
++=-::-#%*+****%@@@@@@@@@@@%@@@@@@@@@@%#***++#%+:::-++
+++%@@*#+++**%@%*@%-#%-+#:=*-=#--%+-@%#@%#**++*+#@@*++
+++#%#+%+**#%%@@@@@%%@%%%%%%%%%%%%%%@@@@@%#**+#*#%@#++
+++#%#*%***%%=@# ## +# =* :* .#. %- @+ @%*%%**#*#%%#+*
++*#%#*%**%%= =: -: :: .: .:  :  -  =  *..#%#*#**%#*++
*******@*#%*                              =%%##%******
*******%*%%+                               %%##%******
*******%*%%-                               %%##%******
###*###@*#%:                               %%#*@######
******#%*%%:                               %%#*@******
*##*##%%*#%:                               %%#*%#####*
******%%*%%:                               %%#*%#*****
######%%*#%:                               %%#*%######
******@##%%:                               #%#*@#*****
****##%#*#%:                               #%#*@%*****
*****#%#*%%:                               #%#*%%***#*
######@**#%:                               #%#*%%#####
**#**#@**#%:                               #%#*%%#****
**#**%@**#%:                               #%#*#%#****
****#%%*##%:                               #%#*#@#**#*
****###**##:                               #%#*#%#**#*
****#%%###%:                               #%###@#****
#####@%####:                               #####@%####
#####@%###%:                               #%###@%####
#####%#####+===============================#####%%####
####%@####**********************************####%%####
##%%%@#++++++++++++++++++++++++++++++++++++++++*%@##%%
%%%#***********+***********++*********************#%%%


            [red]You enter the castle[/red]
""")

    @staticmethod
    def door():
        print(r"""
                                                                       
                                 #@#=++#:                              
                                -*:.....+                              
                            *@-@ #=.....=@---#                         
                           @....-@%.....%*---*@                        
                          .@*....@%@=.=#%#==#%...+                     
                        +..:@@#*%@@@@@@@@@%#@.....@                    
                       %:...*@@@@@%#%*##%@@@@*:=#*@#@                  
                     -:@@*=+*@@%###%% *+#%%#%@@@@=    =                
                    ....-@@@%##**%+=# +=+%@###@@*+..:+%=               
                   *=:-=@@@##+@#+++*@ #+++#%++#@@**##@#@               
                  : :@@@@%##*++=+%%+# +#*+++%++#@@@-....%              
                 +.   @@%@#*++*#++=+# =+##++##++*@@=...:#              
                 #+.=#@@*=++@*+=+*%+# #++%*+++#+=*@@**#@@              
                #@@@@@%##*+:....*-+#@ @%##*+--::-::@@:  .@             
               #----@@@@*+++#*+==**+# +%%+=+#*+#%##@%.   .             
               =+*=*@%#++##+==+#*==+% +=+**+*%*++@@%@-=--#             
               @@%@@@**#*=++%+==++%+# #==+@+++@*+%%#@@-::*-            
               %...@@@*+++%%+=+#+++=* +#*++#*+*%+*%#%=:...%            
              =....-@#+%%++=*%+=+*#+# ++#*+*#++#++#@@#%%*#@            
              %#%+%#@%#+=+%++++*%+=+% *++#*+**++%#*%@=---#:            
               =%@@@@#++@*+=+%=+==### #*=+%++*%++%*##----*%            
             .=....:@#%%++#@*==**+==# =*#++#*+*#++##@@%%%@%            
             #....-+@%+++**++#%+=+*## ++%%++#*=+#+*@@:...:@            
             ##+*#+#@#*#+#++@*++%+++# *+++%++**++%+@#%+-:.@            
             @:    %@%@*++@%+++%+++## ++#++#*=*#=*%#@%#*+*:            
             -    .=@%*+##+++%%+=*#+# #**%**#%*#%*#@@.    @            
             @*+*+*#@==--::.-.......+ ===+=++++++=**@:..+#@            
             @@+--=@@%%@%@@@@@@@%%%@@ +=%+=*#+*#++@#@@@@@@%            
            :*------@%+++@%++%*+*@%+# #++*#+*#=*@+*@@......=           
             @@%##*@@%+##++*@+=+#++*@ +**++#+*@++#+%@@%*+=%            
            @:::=+@%@#%@*+#@++#*+++%# *=**=@#++%+#*#@*....%-           
            %......-@#@#+##++%*++%++# +*==%++%+*%+%#@=.    %           
            @%%*#+%#@%%%@@%%@%#%@##%@ %#@@%@%#@%@@%@@@@@@@@@           
       -#%%#*#+=.                                       +-    : *++    
                       %+     @#@                          :@@@@       
  .  #@- %#**     +   %-  ==                                    .      


    You see a door at the back of the room and you go deeper in the dungeon
""")
    
    @staticmethod
    def goblin():
        print(r"""
             (         .-,
            / )      .' (       ___
           //(.-""\"-/ /\ )   .-"   "-.
           |.'     _`.\// .-'         `..--.
       ..--/-.   .'_` `'-'      ___    ( C\ \
      /   /O\    / O\   |     .'   ```--`-|_/
     /    \_/|   \__/   |    (
    |     _\-' _   __  /      `.
    \ ,--7  `.(_)_.| .'         `.
     | C._)   `.___/' .---._      \           __
     \    |       `. /      `-.    \        ,'  `.
      `--'          Y          \    |       )     `-._.--.__
                    |     \         |    _.'  .--.___.-'`--.\
                    |      |        |\  /    /              `
               _    \      |       /  `v    |
          .'`-' `--._)     /::___.'     \   /
         /    .---.       /   `-.        \ |
      _.' _.-' `._ `-.   /       `.       /
     /.--'        `-. `. \         `-.__.'
                     `.'_/

""")

    @staticmethod
    def zombie():
        print(r"""
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⢀⡆⠀⠀⠀⣀⡀⣀⣀⣰⣋⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⢠⡀⠀⠀⡨⡖⠃⠉⠀⠀⠀⠀⠀⠀⠀⠈⠉⠓⠦⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠈⡷⠋⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⢯⠉⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⡰⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⡇⠀⠆⠀⡠⠒⠋⠓⠢⡄⠀⠀⠀⢀⣀⣀⣀⠀⡸⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⢇⠀⠇⠀⡇⠀⢀⠀⠀⠈⡆⠀⢠⠏⠀⠀⠀⠉⢞⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠘⡄⠀⠀⢇⡀⠉⠀⠀⢸⠀⠀⢸⡀⠀⠀⠀⠁⡼⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠘⣆⠀⠀⠙⠒⠒⠋⠀⠀⠀⡀⠫⠤⠤⢴⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠰⣇⡀⠀⠀⠀⠀⠀⠀⠐⠃⠈⠃⠀⠀⢀⣸⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⡇⠀⡤⢖⠒⠋⡇⠉⠉⠉⡝⠉⢱⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⡇⢸⡁⠙⠒⠒⣃⣠⠤⡄⠉⠉⠁⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⡇⠀⠙⠦⠤⠤⠼⠤⠼⠤⠤⠤⠤⡤
⠀⠀⠀⠀⠀⠀⠀⡨⢽⠳⠶⣒⠢⠤⠤⠤⢄⣀⣀⡼⠀
⠀⠀⠀⢀⠤⠒⠁⠀⠰⣉⢉⠒⠠⣍⡛⡆⠀⠀⠀⠀
⠀⡠⠞⠁⠀⠀⠀⠀⠀⠀⡇⢄⠉⢩⠍⠁⠉⠒⣄⠀
⠘⢒⠀⠀⠀⠀⠀⠀⠀⠀⡏⠓⣜⢾⠀⢰⠀⠀⠀⡜
⠀⠀⠃⠀⠀⠇⠀⠀⠀⠀⡗⢄⠀⣇⠀⠀⡇⠀⠀⡇
⠀⢐⠀⠀⠀⡷⠁⠀⠀⠀⡏⠑⠊⡟⡄⠀⢇⠀⠀⡅
⠀⢸⠀⠀⠀⡇⠀⠀⠀⢀⠗⠠⡀⠀⢣⠀⠀⣇⠀⢸⠀⠀⠀⠀
⠀⢸⠀⠀⠀⡇⠀⠀⠀⢸⡐⠆⡀⠑⠢⣳⠀⢱⠀⠘⡆⠀⠀⠀
⠀⢸⠀⠀⠀⡇⠀⠀⠀⠀⠈⢦⠀⠈⢢⠃⠀⠀⢱⠀⢸⡀⠀⠀
⠀⢸⠀⠀⠀⢳⠀⠀⠀⠀⠀⠀⠈⢆⡎⠀⠀⠀⠀⢳⠀⢿⠀⠀
⠀⢸⠀⠀⠀⢸⠀⠀⠀⠀⠀⠀⠀⠀⠁⠀⠀⠀⠀⠀⡇⠀⣇⠀
⠀⠀⠀⠀
""")

    @staticmethod
    def skeleton():
        print(r"""
                             _.--""-._
  .                         ."         ".
 / \    ,^.         /(     Y             |      )\
/   `---. |--'\    (  \__..'--   -   -- -'""-.-'  )
|        :|    `>   '.     l_..-------.._l      .'
|      __l;__ .'      "-.__.||_.-'v'-._||`"----"
 \  .-' | |  `              l._       _.'
  \/    | |                   l`^^'^^'j
        | |                _   \_____/     _
        j |               l `--__)-'(__.--' |
        | |               | /`---``-----'"1 |  ,-----.
        | |               )/  `--' '---'   \'-'  ___  `-.
        | |              //  `-'  '`----'  /  ,-'   I`.  \
      _ L |_            //  `-.-.'`-----' /  /  |   |  `. \
     '._' / \         _/(   `/   )- ---' ;  /__.J   L.__.\ :
      `._;/7(-.......'  /        ) (     |  |            | |
      `._;l _'--------_/        )-'/     :  |___.    _._./ ;
        | |                 .__ )-'\  __  \  \  I   1   / /
        `-'                /   `-\-(-'   \ \  `.|   | ,' /
                           \__  `-'    __/  `-. `---'',-'
                              )-._.-- (        `-----'

""")

    @staticmethod
    def chest():
        print(r"""
                    ____...------------...____
               _.-"` /o/__ ____ __ __  __ \o\_`"-._
             .'     / /                    \ \     '.
             |=====/o/======================\o\=====|
             |____/_/________..____..________\_\____|
             /   _/ \_     <_o#\__/#o_>     _/ \_   \
             \________________\####/________________/
              |===\!/========================\!/===|
              |   |=|          .---.         |=|   |
              |===|o|=========/     \========|o|===|
              |   | |         \() ()/        | |   |
              |===|o|======{'-.) A (.-'}=====|o|===|
              | __/ \__     '-.\uuu/.-'    __/ \__ |
              |        |==== .'.'^'.'.====|        |
              |  _\o/   __  {.' __  '.} _   _\o/  _|
              `""""-""""""""""""""""""""""""""-""""`
""")

    @staticmethod
    def drake():
        print(r"""
             __                  __
            ( _)                ( _)
           / / \\              / /\_\_
          / /   \\            / / | \ \
         / /     \\          / /  |\ \ \
        /  /   ,  \ ,       / /   /|  \ \
       /  /    |\_ /|      / /   / \   \_\
      /  /  |\/ _ '_| \   / /   /   \    \\
     |  /   |/  0 \0\    / |    |    \    \\
     |    |\|      \_\_ /  /    |     \    \\
     |  | |/    \.\ o\o)  /      \     |    \\
     \    |     /\\`v-v  /        |    |     \\
      | \/    /_| \\_|  /         |    | \    \\
      | |    /__/_ `-` /   _____  |    |  \    \\
      \|    [__]  \_/  |_________  \   |   \    ()
       /    [___] (    \         \  |\ |   |   //
      |    [___]                  |\| \|   /  |/
     /|    [____]                  \  |/\ / / ||
    (  \   [____ /     ) _\      \  \    \| | ||
     \  \  [_____|    / /     __/    \   / / //
     |   \ [_____/   / /        \    |   \/ //
     |   /  '----|   /=\____   _/    |   / //
  __ /  /        |  /   ___/  _/\    \  | ||
 (/-(/-\)       /   \  (/\/\)/  |    /  | /
               (/\/\)           /   /   //
                      _________/   /    /
                     \____________/    (
""")

    @staticmethod
    def demon():
        print(r"""
                            ,-.                               
       ___,---.__          /'|`\          __,---,___          
    ,-'    \`    `-.____,-'  |  `-.____,-'    //    `-.       
  ,'        |           ~'\     /`~           |        `.      
 /      ___//              `. ,'          ,  , \___      \    
|    ,-'   `-.__   _         |        ,    __,-'   `-.    |    
|   /          /\_  `   .    |    ,      _/\          \   |   
\  |           \ \`-.___ \   |   / ___,-'/ /           |  /  
 \  \           | `._   `\\  |  //'   _,' |           /  /      
  `-.\         /'  _ `---'' , . ``---' _  `\         /,-'     
     ``       /     \    ,='/ \`=.    /     \       ''          
             |__   /|\_,--.,-.--,--._/|\   __|                  
             /  `./  \\`\ |  |  | /,//' \,'  \                  
            /   /     ||--+--|--+-/-|     \   \                 
           |   |     /'\_\_\ | /_/_/`\     |   |                
            \   \__, \_     `~'     _/ .__/   /            
             `-._,-'   `-._______,-'   `-._,-'
""")

    @staticmethod
    def menu():
        print(r"""
 _______  _______  _                
|       ||  ____ \| \    /||\     /|
| || || || |    \/|  \  | || |   | |
| || || || |__    |   \ | || |   | |
| ||_|| ||  __|   | |\ \| || |   | |
| |   | || |      | | \   || |   | |
| /   \ || |____/\| |  \  || |___| |
|/     \||_______/|/    \_||_______|
""")

    @staticmethod
    def village():
        print(r"""
         _________ _        _        _______  _______  _______ 
|\     /|\__   __/| \      | \      |  ___  ||  ____ \|  ____ \
| |   | |   | |   | |      | |      | |   | || |    \/| |    \/
| |   | |   | |   | |      | |      | |___| || |      | |__    
| |   | |   | |   | |      | |      |  ___  || | ____ |  __|   
 \ \_/ /    | |   | |      | |      | |   | || | \_  || |      
  \   /  ___| |___| |____/\| |____/\| |   | || |___| || |____/\
   \_/   \_______/|_______/|_______/|/     \||_______||_______/
""")

    @staticmethod
    def inventory():
        p(r"""[red]
_________ _                 _______  _       _________ _______  _______          
\__   __/| \    /||\     /||  ____ \| \    /|\__   __/|  ___  ||  ____ ||\     /|
   | |   |  \  | || |   | || |    \/|  \  | |   | |   | |   | || |    ||| \   / |
   | |   |   \ | || |   | || |__    |   \ | |   | |   | |   | || |____|| \ \_/ / 
   | |   | |\ \| || |   | ||  __|   | |\ \| |   | |   | |   | ||     __|  \   /  
   | |   | | \   | \ \_/ / | |      | | \   |   | |   | |   | || |\ \      | |   
___| |___| |  \  |  \   /  | |____/\| |  \  |   | |   | |___| || | \ \__   | |   
\_______/|/    \_|   \_/   |_______/|/    \_|   |_|   |_______||/   \__/   \_/
[/red]
""")

    @staticmethod
    def bye():
        p(r"""[red]
 ______            _______   ______            _______ 
|  ___ \ |\     /||  ____ \ |  ___ \ |\     /||  ____ \
| |   | || \   / || |    \/ | |   | || \   / || |    \/
| |__/ /  \ \_/ / | |__     | |__/ /  \ \_/ / | |__    
|  __ |    \   /  |  __|    |  __ |    \   /  |  __|   
| |  \ \    | |   | |       | |  \ \    | |   | |      
| |___| |   | |   | |____/\ | |___| |   | |   | |____/\
|______/    \_/   |_______/ |______/    \_/   |_______/
[/red]""")

    @staticmethod
    def shop():
        p(r"""[blue]
 _______           _______  _______ 
|  ____ \|\     /||  ___  ||  ___  |
| |    \/| |   | || |   | || |   | |
| |_____ | |___| || |   | || |___| |
|_____  ||  ___  || |   | ||  _____|
      | || |   | || |   | || |      
/\____| || |   | || |___| || |      
\_______||/     \||_______||/       
[/blue]
""")

    @staticmethod
    def swords():
        p(r"""[red]
 _____  _    _  _________________  _____ 
/  ___|| |  | ||  _  | ___ \  _  \/  ___|
\ `--. | |  | || | | | |_/ / | | |\ `--. 
 `--. \| |/\| || | | |    /| | | | `--. \
/\__/ /\  /\  /\ \_/ / |\ \| |/ / /\__/ /
\____/  \/  \/  \___/\_| \_|___/  \____/
[/red]
""")

    @staticmethod
    def knives():
        p(r"""[green]
 _   __ _   _ _____ _   _ _____ _____ 
| | / /| \ | |_   _| | | |  ___/  ___|
| |/ / |  \| | | | | | | | |__ \ `--. 
|    \ | . ` | | | | | | |  __| `--. \
| |\  \| |\  |_| |_\ \_/ / |___/\__/ /
\_| \_/\_| \_/\___/ \___/\____/\____/ 
[/green]
""")

    @staticmethod
    def hammers():
        p(r"""[yellow1]
 _   _   ___  ___  ______  ___ ___________  _____ 
| | | | / _ \ |  \/  ||  \/  ||  ___| ___ \/  ___|
| |_| |/ /_\ \| .  . || .  . || |__ | |_/ /\ `--. 
|  _  ||  _  || |\/| || |\/| ||  __||    /  `--. \
| | | || | | || |  | || |  | || |___| |\ \ /\__/ /
\_| |_/\_| |_/\_|  |_/\_|  |_/\____/\_| \_|\____/ 
[/yellow1]
""")

    @staticmethod
    def sticks():
        p(r"""[blue]
 _____ _____ _____ _____  _   __ _____ 
/  ___|_   _|_   _/  __ \| | / //  ___|
\ `--.  | |   | | | /  \/| |/ / \ `--. 
 `--. \ | |   | | | |    |    \  `--. \
/\__/ / | |  _| |_| \__/\| |\  \/\__/ /
\____/  \_/  \___/ \____/\_| \_/\____/ 
[/blue]
""")

    @staticmethod
    def shields():
        p(r"""[dark_magenta]
 _____ _   _ _____ _____ _    ______  _____ 
/  ___| | | |_   _|  ___| |   |  _  \/  ___|
\ `--.| |_| | | | | |__ | |   | | | |\ `--. 
 `--. \  _  | | | |  __|| |   | | | | `--. \
/\__/ / | | |_| |_| |___| |___| |/ / /\__/ /
\____/\_| |_/\___/\____/\_____/___/  \____/
[/dark_magenta]
""")

    @staticmethod
    def helmets():
        p(r"""[purple4]
 _   _  _____ _     ___  ___ _____ _____ _____ 
| | | ||  ___| |    |  \/  ||  ___|_   _/  ___|
| |_| || |__ | |    | .  . || |__   | | \ `--. 
|  _  ||  __|| |    | |\/| ||  __|  | |  `--. \
| | | || |___| |____| |  | || |___  | | /\__/ /
\_| |_/\____/\_____/\_|  |_/\____/  \_/ \____/ 
[/purple4]
""")

    @staticmethod
    def chestplates():
        p(r"""[purple4]
 _____  _   _  _____ _____ ___________ _       ___ _____ _____ _____ 
/  __ \| | | ||  ___/  ___|_   _| ___ \ |     / _ \_   _|  ___/  ___|
| /  \/| |_| || |__ \ `--.  | | | |_/ / |    / /_\ \| | | |__ \ `--. 
| |    |  _  ||  __| `--. \ | | |  __/| |    |  _  || | |  __| `--. \
| \__/\| | | || |___/\__/ / | | | |   | |____| | | || | | |___/\__/ /
 \____/\_| |_/\____/\____/  \_/ \_|   \_____/\_| |_/\_/ \____/\____/
[/purple4]
""")

    @staticmethod
    def leggings():
        p(r"""[purple4]
 _      _____ _____ _____ _____ _   _ _____  _____ 
| |    |  ___|  __ \  __ \_   _| \ | |  __ \/  ___|
| |    | |__ | |  \/ |  \/ | | |  \| | |  \/\ `--. 
| |    |  __|| | __| | __  | | | . ` | | __  `--. \
| |____| |___| |_\ \ |_\ \_| |_| |\  | |_\ \/\__/ /
\_____/\____/ \____/\____/\___/\_| \_/\____/\____/
[/purple4]
""")

    @staticmethod
    def boots():
        p(r"""[purple4]
______  _____  _____ _____ _____ 
| ___ \|  _  ||  _  |_   _/  ___|
| |_/ /| | | || | | | | | \ `--. 
| ___ \| | | || | | | | |  `--. \
| |_/ /\ \_/ /\ \_/ / | | /\__/ /
\____/  \___/  \___/  \_/ \____/
[/purple4]
""")

    @staticmethod
    def potions():
        p(r"""[bright_green]
______ _____ _____ _____ _____ _   _  _____ 
| ___ \  _  |_   _|_   _|  _  | \ | |/  ___|
| |_/ / | | | | |   | | | | | |  \| |\ `--. 
|  __/| | | | | |   | | | | | | . ` | `--. \
| |   \ \_/ / | |  _| |_\ \_/ / |\  |/\__/ /
\_|    \___/  \_/  \___/ \___/\_| \_/\____/ 
[/bright_green]
""")

    #endregion


    #region Menu
    @staticmethod
    def print_new_game():
        print("Hi advanturer, what do you want to do ?")
        p(" - 1 : [blue]Load a save[/blue]")
        p(" - 2 : [green]Start a new game[/green]")

    

    @staticmethod
    def print_class():
        p(" \n Choose your class : \n")
        p(" - 1 : [red]Warrior[/red]")
        p(" - 2 : [blue]Mage[/blue]")
        p(" - 3 : [green]Thief[/green]")
        p(" - 4 : [yellow1]Colossus[/yellow1] \n \n")

    @staticmethod
    def print_char(name,charclass):
        p("Hello " + name + " the " + charclass + " !")

    @staticmethod
    def print_hub():
        p(" \n Where do you want to go ? : \n")
        p(" - 1 : [red]See the inventory[/red]")
        p(" - 2 : [blue]Go to shop[/blue]")
        p(" - 3 : [green]Endless cave[/green]")
        p(" - 4 : [yellow1]Dungeon (INSANE!)[/yellow1] \n \n \n")
        p("\n - 5 : [bright_green]Save and quit[/bright_green]")

    @staticmethod
    def quit_inventory():
        p(" \n Do you want to quit inventory ? \n")
        p(" - 1 : [green1]Yes ![/green1]")
        p(" - 2 : [red]Never (If you are angry)[/red]")

    @staticmethod
    def shop_categories():
        p(" \n What do you want to buy ? \n")
        p(" - 1 : [red]Swords[/red]")
        p(" - 2 : [green]Knives[/green]")
        p(" - 3 : [yellow1]Hammers[/yellow1]")
        p(" - 4 : [blue]Mage's sticks[/blue]")
        p(" - 5 : [dark_magenta]Shields[/dark_magenta]")
        p(" - 6 : [purple4]Helmet[/purple4]")
        p(" - 7 : [purple4]Chestplate[/purple4]")
        p(" - 8 : [purple4]Leggings[/purple4]")
        p(" - 9 : [purple4]Boots[/purple4]")
        p(" - 10 : [bright_green]Potions[/bright_green]")
        p(" - 11 : [grey66]Nevermind[/grey66] \n \n")

    @staticmethod
    def cant_afford():
        p("You can't afford this item (automatically returning to the shop)")

    @staticmethod
    def wrong_class(char_class):
        p("A "+char_class+" can't use this type of weapon mate (automatically returning to the shop)")

    @staticmethod
    def nevermind():
        p(" - 7 : Nevermind")

    @staticmethod
    def already_have(item):
        p("\n You already have a "+item.item_name+" Do you want to replace it ? \n ")
        p(" - 1 : Yes")
        p(" - 2 : No")

    @staticmethod
    def difficulty():
        p("\n Choose how much you want to suffer : \n")
        p(" - 1 : [green]Not too much[/green] (5 rooms)")
        p(" - 2 : [yellow1]Just a bit[/yellow1] (10 rooms)")
        p(" - 3 : [dark_orange]As much as you can[/dark_orange] (20 rooms)")
        p(" - 4 : [red]A love SM[/red] (50 rooms)")

    @staticmethod
    def tier():
        tier = ["Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic"]

    @staticmethod
    def open_chest():
        p("Do you want to open it ?")
        p(" - 1 : [green1]Yes[/green1]")
        p(" - 2 : [red]No ! (I'm a boring guy)[/red]")

    @staticmethod
    def next_room_infinite():
        p("You finished this room , what do you want to do now ?")
        p(" - 1 Return to the village")
        p(" - 2 Continue the endless cave (Be carefull if you die you lost everything)")

    @staticmethod
    def next_room_dungeon():
        p("You finished this room , are you ready for the next one ? (You don't really have the choice)")
        p(" - 1 : Yes")
        p(" - 2 : Second answer")

    @staticmethod
    def which_attack(attack1, attack2):
        p(f"Which attack do you want to do ?")
        p(f" - 1 {attack1.name}")
        p(f" - 2 {attack2.name}")
        p(f" - 3 Drink a potion")

    @staticmethod
    def wrong_input_class():
        p("Wrong entry, choose a number between 1 and 4.")

    @staticmethod
    def wrong_input_inventory():
        p("Wrong entry, can you read ?")

    @staticmethod
    def wrong_input_shop():
        p("Wrong entry (automatically returning to the shop)")

    #region Fight
    @staticmethod
    def facing_enemy(enemy):
        p(f"You're facing a {enemy}, what are you going to do ?")

    @staticmethod
    def no_potion():
        p("You don't have any potions (choose an attack)")

    @staticmethod
    def finish_fight(drop):
        print(f"The enemy dropped a {drop} do you want to take it ?(It will replace your current one if you have one)")
        print(" - 1 Yes")
        print(" - 2 No")

    @staticmethod
    def loose_fight():
        p("You have been defeated, what a shame ! (returning to the village)")

    @staticmethod
    def choice_chest(loot):
        print(f"This chest contain a {loot} Do you want to take it (and sell your equipped one ?)")
        print("1 - Yes")
        print("2 - No")

    #endregion